...
export default {
  ...
  data() {
        return{
            ...
            checkboxValue: false,
            empty_message: "No tasks related to work to do today yet...Have a nice day..:)",
        }
  },
  async beforeCreate(){
    try{
        const res = await axios.get('http://localhost:3000/category/work' )
  ...
  methods: {
        ...
        async DeleteFromList(id){
            ...
            if(!this.show_all_button){
                  ...
                  const res = await axios.get('http://localhost:3000/all/work')
                  this.list = res.data.rows
              }
        },
        getDate: function(){
        ...
        async show_all(){
            if(!this.show_all_button){
                const result = await axios.get('http://localhost:3000/all/work' )
                ...
                const result = await axios.get('http://localhost:3000/category/work' )
                this.list = result.data.rows    
