...
export default {
  ...
  data() {
        return{
            ...
            checkboxValue: false,
            empty_message: "No tasks with books to do today yet...Have a nice day..:)",
        }
  },
  async beforeMount(){
    try{
        const res = await axios.get('http://localhost:3000/category/books' )
  ...
  methods: {
        ...
        async show_all(){
            if(!this.show_all_button){
                ...
                const result = await axios.get('http://localhost:3000/all/books')
                ...
                const result = await axios.get('http://localhost:3000/category/books')
                this.list = result.data.rows    
